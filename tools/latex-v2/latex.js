    // 🚀 Init 🚀
    var cmEditor = null;
    var nativeEditor = null;

    document.addEventListener('DOMContentLoaded', function () {
        if (typeof Comments !== 'undefined') Comments.init('LaTeXEditor');

        var textarea = document.getElementById('latex-editor');
        nativeEditor = textarea;
        
        try {
            if (typeof CodeMirror === 'undefined') throw new Error('CodeMirror is not loaded');
    ...
    function toggleZenMode() {
        ...
        setTimeout(function() {
            ...
            cmEditor.on('change', function () {
                handleContentChange(cmEditor.getValue());
            });
            ...
            console.error("Lỗi khởi tạo CodeMirror, chuyển sang chế độ gõ cơ bản:", err);
            ...
            textarea.style.backgroundColor = 'rgba(255,255,255,0.05)';
            textarea.style.color = 'inherit';
            textarea.style.border = '1px solid rgba(92,225,230,0.2)';
        ...
        function setEditorValue(val) {
            if (cmEditor) setEditorValue(val);
            else if (nativeEditor) nativeEditor.value = val;
        }

        function getEditorValue() {
            if (cmEditor) return cmEditor.getValue();
            if (nativeEditor) return nativeEditor.value;
            return '';
        }
        ...
        var transferData = localStorage.getItem('smp_latex_transfer');
        if (transferData) {
            var curr = getEditorValue();
            var prefix = curr ? '\n\n' : '';
            var insertStr = transferData;
            // Tránh bọc display math nhiều lần
            if (!insertStr.trim().startsWith('\\[') && !insertStr.trim().startsWith('
        // Intercept keys for LaTeX auto-close brackets
        cmEditor.on('keydown', function(cm, e) {
            if (e.key === '[' || e.key === '(' || e.key === '{') {
                var cur = cm.getCursor();
    ...
    function toggleZenMode() {
        ...
        setTimeout(function() {
            ...
            setEditorValue(curr + prefix + insertStr);
            localStorage.removeItem('smp_latex_transfer');
            setTimeout(doRender, 100);
        }
        ...
        if (typeof Split !== 'undefined') {
            ...
            } catch (e) { console.warn("Split.js failed:", e); }
        }
    });
        // Intercept keys for LaTeX auto-close brackets
        cmEditor.on('keydown', function(cm, e) {
